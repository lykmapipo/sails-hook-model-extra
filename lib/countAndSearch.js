...
/**
 * @function
 * @requires countAndFind
 * @description Count hits and perform free search on record(s) using like 
 *              criteria modifier on model attributes
...
module.exports = function(model) {
    ...
    /**
     * @function
     * @description Count hits and perform free search on record(s) using like criteria modifier
     * @param  {String}   searchTerm A search term to match agaist record(s)
     * @param  {Function} callback  A callback to be invoked on result
     */
    function countAndSearch(searchTerm, callback) {
        ...
        //build a countAndFind query using criterias 
        var query = model.countAndFind(criterias);
    ...
    //attach countAndSearch
    //to the model
    model.countAndSearch = countAndSearch;
}
